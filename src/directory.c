                 ...
                 void* mapped_file)
{
    ...
    struct inode* created_inode_ptr = get_inode_ptr(created_inode_idx, 
                                                    filesys_data, 
                                                    mapped_file);
    if (created_inode_idx != 0) 
    {
        struct inode* parent_inode_ptr = get_inode_ptr(parent_inode_idx, 
                                                       filesys_data, 
                                                       mapped_file);
        size_t curr_parent_dir_links_cnt = get_dir_links_cnt(parent_inode_ptr, 
                                                             filesys_data, 
                                                             mapped_file);
